...
def sampling(start_env, max_program_len, model, timeout, top_p, temperature):
    ...
    while time.time() < end_time:
        ...
        for _ in range(max_program_len):
            ...
            statement_probs = torch.exp(statement_log_probs / temperature)
            statement_probs *= torch.cumsum(statement_log_probs[batch_indices, statement_pred], 1) <= top_p
            statement_probs /= torch.sum(statement_probs, 1)[:, None]
            next_statement = torch.multinomial(statement_probs, 1).squeeze().detach().cpu().numpy()

            for i, env in enumerate(envs):
                envs[i] = env.step_safe(index_to_statement[next_statement[i]])
                statements[i].append(next_statement[i])
...
def solve_problem_worker(data):
    ...
    solution = sampling(env, max_program_len, model, timeout, top_p=0.8, temperature=1)
